...
public:
  handler(boost::asio::io_service& io)
    : m_strand(io),
      ...
      m_timer2(io, boost::posix_time::seconds(2)),
      m_count(0)
  {
  ...
  void message2()
  {
    if (m_count < 20)
    {
      ...
      m_timer2.expires_at(m_timer2.expires_at() + boost::posix_time::seconds(2));
      m_timer2.async_wait(m_strand.wrap(boost::bind(&handler::message2, this)));
    }
    if (m_count == 13)
    {
      std::cout << "Timer 1:2: " << m_count << "\n";
      m_timer1.expires_at(m_timer1.expires_at() + boost::posix_time::seconds(9));
      m_timer1.async_wait(m_strand.wrap(boost::bind(&handler::message1, this)));
    }
  }
