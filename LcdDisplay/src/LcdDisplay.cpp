...
/**
 * Constructor
 * @param szAddres is the I2C address of the device
 *
...
void LcdDisplay::init() {
   ...
   if (-1 != m_nDeviceFD) {
      try {
         ...
         write(
               K_LCD_FUNCTIONSET | K_LCD_2LINE | K_LCD_5x8DOTS
                     | K_LCD_4BITMODE);
 ...
 * @param szCol is the column number
 *
 */
//---------------------------------------------------
void LcdDisplay::displayStringAtPosition(const char* pData, char szLine,
      char szCol) {
 ...
 */
//---------------------------------------------------
void LcdDisplay::setCursorAtPosition(char szLine, char szCol, bool isVisible,
      bool isBlinking) {
   ...
   // Now move the cursor
   for (unsigned char szIndex = 0; szIndex < K_LCD_MAX_CHAR_PER_LINE;
         szIndex++) {
      if (szIndex == szCol) {
         break;
      }
